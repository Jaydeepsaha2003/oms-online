...
/** A GSTIN's first two digits identify the registered state, never the city. */
export function gstStateFromGstin(gstin?: string | null): string | null {
  const value = gstin?.trim() ?? '';
  if (!/^\d{2}[A-Z0-9]{13}$/i.test(value)) return null;
  return GST_STATE_BY_CODE[value.slice(0, 2)] ?? null;
}
