...
export class OrdersController {
  ...
  async deletePhoto(@Param('photoId', ParseIntPipe) photoId: number, @CurrentUser() user?: { email?: string | null; roles?: string[] }) {
    await this.orders.deletePhoto(photoId, user?.email ?? null, user?.roles ?? []);
    return { ok: true };
  }
