...
export function OrdersPage() {
  ...
  const columns: DataColumn<OrderDto>[] = [
    { id: 'code', label: 'Order #', cell: (o) => <span className="font-mono text-xs font-medium">{o.code ?? `#${o.id}`}</span> },
    { id: 'customer', label: 'Customer', cell: (o) => <span className="font-medium">{o.customerName}</span> },
    { id: 'agent', label: 'Agent', cell: (o) => o.agentName ?? '—' },
    { id: 'orderDate', label: 'Order date', cell: (o) => <span className="whitespace-nowrap">{formatDateShort(o.orderDate)}</span> },
    { id: 'completion', label: 'Completion', cell: (o) => <span className="whitespace-nowrap">{o.completionDate ? formatDateShort(o.completionDate) : '—'}</span> },
    {
      id: 'priority',
      label: 'Priority',
      cell: (o) =>
        o.priority === 'URGENT' ? <span className="font-semibold text-rose-600">URGENT</span> : (o.priority ?? '—'),
    },
    ...
    { id: 'total', label: 'Total', align: 'right', cell: (o) => <span className="font-semibold tabular-nums">₹{o.totalRate.toLocaleString()}</span> },
    {
      ...
      label: 'Status',
      cell: (o) => (
        <span className={`rounded px-1.5 py-0.5 text-xs font-medium ring-1 ${STATUS_STYLE[o.status] ?? 'bg-muted text-muted-foreground ring-border'}`}>
          {o.status}
        </span>
      ),
    },
