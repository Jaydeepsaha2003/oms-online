  ...
  return (
    <div className="space-y-2.5 font-sans">
      ...
      <div className="bg-card font-poppins flex flex-wrap items-center gap-2 rounded-[4px] border p-2.5 shadow-sm sm:p-3">
        ...
        <p className="text-muted-foreground hidden text-[12px] font-medium sm:block">GST rate per customer × product category.</p>
...
function RatesList() {
  ...
  const columns: DataColumn<GstRateDto>[] = [
    ...
    { id: 'category', label: 'Product category', cell: (r) => <span className={TEXT_CELL}>{r.category}</span> },
    { id: 'rate', label: 'GST rate', align: 'right', cell: (r) => <span className="text-[13px] font-bold tabular-nums text-emerald-700 dark:text-emerald-400">{pct(r.rate)}</span> },
    {
      id: 'updated',
      label: 'Last updated',
      cell: (r) => (
              ...
              <div key={customerName} className="bg-card overflow-hidden rounded-[4px] border shadow-sm">
                ...
                <div className="divide-y divide-slate-200 dark:divide-white/10">
                  {rates.map((r) => (
                    <div
                      key={r.id}
                      className={cn('px-3 py-2', can('gstrate:update') && 'cursor-pointer active:bg-muted')}
                      onClick={can('gstrate:update') ? () => setEditing(r) : undefined}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-[13px] font-bold text-slate-800 dark:text-slate-200">{r.category}</p>
                        <span className="text-[13px] font-bold tabular-nums text-emerald-700 dark:text-emerald-400">{pct(r.rate)}</span>
                      </div>
